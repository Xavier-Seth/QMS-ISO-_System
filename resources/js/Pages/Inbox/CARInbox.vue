...
function submitReject() {
  ...
  router.post(
    `/inbox/car/${rejectRecordId.value}/reject`,
    {
    ...
    },
    {
      ...
      onError: (errors) => {
        console.log(errors)
        alert(errors.rejection_reason || 'Failed to reject CAR.')
      },
    }
  )
}
...
--------------------------- */
function approve(id) {
  router.post(`/inbox/car/${id}/approve`, {}, {
    preserveScroll: true,
  })
}
    ...
    >
      <div class="w-full max-w-lg rounded-2xl bg-white p-6 shadow-xl">
        ...
        <p class="mt-1 text-sm text-slate-500">
          Please provide a reason for rejecting this CAR.
        </p>
    ...
    <div class="space-y-6 p-6">
      ...
      <div class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        <div class="bg-gradient-to-r from-slate-900 to-slate-800 px-6 py-5">
          <h1 class="text-xl font-semibold text-white">CAR Inbox</h1>
          <p class="mt-1 text-sm text-slate-300">
            Review submitted CAR records from departments and users.
          </p>
        </div>
        ...
        <div class="overflow-x-auto">
          <table class="min-w-full text-sm">
            <thead class="border-b border-slate-200 bg-slate-50">
              <tr class="text-left">
                <th class="px-5 py-3 font-semibold text-slate-700">CAR No.</th>
                <th class="px-5 py-3 font-semibold text-slate-700">Ref. No.</th>
                <th class="px-5 py-3 font-semibold text-slate-700">Dept./Section</th>
            ...
            <tbody>
              ...
              <tr v-if="!records.data.length">
                <td colspan="9" class="px-5 py-8 text-center text-sm text-slate-500">
                  No submitted CAR records found for this status.
                </td>
              </tr>
            </tbody>
          </table>
