...
function applyFilters() {
    ...
    router.get(
        "/documents",
        {
            ...
            hasUploads:
                hasUploads.value !== "All" ? hasUploads.value : undefined,
...
const normalized = computed(() => {
    ...
    if (fileType.value !== "All") {
        ...
        rows = rows.filter((r) =>
            (r.file_type || "").toLowerCase().includes(ft),
        );
    }
    ...
    const byLatest = (a, b) => {
        const da = a.latest_upload_at
            ? new Date(a.latest_upload_at).getTime()
            : 0;
        const db = b.latest_upload_at
            ? new Date(b.latest_upload_at).getTime()
            : 0;
        return db - da;
    };
...
const grouped = computed(() => {
    ...
    for (const r of normalized.value) {
        ...
        if (n !== null && n >= 1 && n <= 61)
            groups["R-QMS-001 to R-QMS-061"].push(r);
        else if (n !== null && n >= 100 && n <= 112)
            groups["R-QMS-100 to R-QMS-112"].push(r);
...
function pillClass(text) {
    ...
    if (t.includes("physical") && t.includes("electronic"))
...
<template>
    <AdminLayout>
        <div class="p-6 space-y-6">
            ...
            >
                <div
                    class="px-4 py-2 bg-gradient-to-r from-slate-900 to-slate-800"
                >
                    <div
                    ...
                    >
                        <div>
                            <h1
                                class="text-base sm:text-x1 font-semibold text-white"
                            >
                                Documents
                            </h1>
                            <p class="text-xs text-slate-300 mt-1">
                                Find documents by code (R-QMS-###) or name. Open
                                a code to view all uploads under it.
                            </p>
                        </div>
                ...
                <div class="px-4 py-2">
                    <div class="grid grid-cols-1 md:grid-cols-12 gap-3">
                        ...
                        <div class="md:col-span-6">
                            ...
                            <div class="mt-1 relative">
                                <input
                                    ...
                                    class="w-full rounded-sm border border-slate-200 px-4 py-1.5 pr-10 focus:outline-none focus:ring-2 focus:ring-slate-300"
                                />
                                <div
                        ...
                        <div class="md:col-span-2">
                            ...
                            <select
                                v-model="fileType"
                                class="mt-1 w-full rounded-xl border border-slate-200 px-3 py-1.5 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-slate-300"
                            >
                        ...
                        >
                            <div class="flex items-start justify-between gap-3">
                                <div class="min-w-0">
                                    <div class="flex items-center gap-2">
                                        <span
                                            class="text-xs font-semibold text-slate-900 bg-slate-100 rounded-md px-2 py-1"
                                        >
                                            {{ row.code }}
                                        </span>
                                        <span
                                            class="text-xs rounded-full px-2 py-1 ring-1"
                                            :class="pillClass(row.file_type)"
                                        >
                                            {{ row.file_type || "—" }}
                                        </span>
                                    </div>
                                    ...
                                    <div
                                        class="mt-2 font-semibold text-slate-900 truncate"
                                    >
                                        {{ row.name }}
                                    </div>
                                ...
                                <span
                                    class="text-xs rounded-full px-2 py-1 ring-1 whitespace-nowrap"
            ...
            <!-- Table View -->
            <div
                v-if="view === 'table' && normalized.length"
            ...
            >
                            ...
                            >
                                ...
                                <td class="px-4 py-3">
                                    <span
                                        class="text-xs rounded-full px-2 py-1 ring-1"
                                        :class="
                                            statusClass(row.documents_count)
                                        "
                                    >
                ...
                <div class="px-4 py-3 text-xs text-slate-500">
                    Tip: Search “OFI” or “DCR” to jump directly to those
                    document types.
                </div>
            </div>
        </div>
